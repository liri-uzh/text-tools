#!/usr/bin/env python3
# -*- coding: utf-8 -*-

# this list is taken from
# http://raw.githubusercontent.com/gosia-malgosia/german-stop-words/refs/heads/master/german-stop-words.txt
# related blog post: https://medium.com/idealo-tech-blog/common-pitfalls-with-the-preprocessing-of-german-text-for-nlp-3cfb8dc19ebe
STOP_WORDS = {
    "de": frozenset([
        "aber",
        "alle",
        "allem",
        "allen",
        "aller",
        "alles",
        "als",
        "also",
        "am",
        "an",
        "ander",
        "andere",
        "anderem",
        "anderen",
        "anderer",
        "anderes",
        "anderm",
        "andern",
        "anders",
        "auch",
        "auf",
        "aus",
        "bei",
        "bin",
        "bis",
        "bist",
        "da",
        "damit",
        "dann",
        "das",
        "dass",
        "dasselbe",
        "dazu",
        "daß",
        "dein",
        "deine",
        "deinem",
        "deinen",
        "deiner",
        "deines",
        "dem",
        "demselben",
        "den",
        "denn",
        "denselben",
        "der",
        "derer",
        "derselbe",
        "derselben",
        "des",
        "desselben",
        "dessen",
        "dich",
        "die",
        "dies",
        "diese",
        "dieselbe",
        "dieselben",
        "diesem",
        "diesen",
        "dieser",
        "dieses",
        "dir",
        "doch",
        "dort",
        "du",
        "durch",
        "ein",
        "eine",
        "einem",
        "einen",
        "einer",
        "eines",
        "einig",
        "einige",
        "einigem",
        "einigen",
        "einiger",
        "einiges",
        "einmal",
        "er",
        "es",
        "etwas",
        "euch",
        "euer",
        "eure",
        "eurem",
        "euren",
        "eurer",
        "eures",
        "für",
        "gegen",
        "gewesen",
        "hab",
        "habe",
        "haben",
        "hat",
        "hatte",
        "hatten",
        "hier",
        "hin",
        "hinter",
        "ich",
        "ihm",
        "ihn",
        "ihnen",
        "ihr",
        "ihre",
        "ihrem",
        "ihren",
        "ihrer",
        "ihres",
        "im",
        "in",
        "indem",
        "ins",
        "ist",
        "jede",
        "jedem",
        "jeden",
        "jeder",
        "jedes",
        "jene",
        "jenem",
        "jenen",
        "jener",
        "jenes",
        "jetzt",
        "kann",
        "kein",
        "keine",
        "keinem",
        "keinen",
        "keiner",
        "keines",
        "können",
        "könnte",
        "machen",
        "man",
        "manche",
        "manchem",
        "manchen",
        "mancher",
        "manches",
        "mein",
        "meine",
        "meinem",
        "meinen",
        "meiner",
        "meines",
        "mich",
        "mir",
        "mit",
        "muss",
        "musste",
        "nach",
        "nicht",
        "nichts",
        "noch",
        "nun",
        "nur",
        "ob",
        "oder",
        "ohne",
        "sein",
        "seine",
        "seinem",
        "seinen",
        "seiner",
        "seines",
        "selbst",
        "sich",
        "sie",
        "sind",
        "so",
        "solche",
        "solchem",
        "solchen",
        "solcher",
        "solches",
        "soll",
        "sollte",
        "sondern",
        "sonst",
        "um",
        "und",
        "uns",
        "unse",
        "unsem",
        "unsen",
        "unser",
        "unses",
        "unter",
        "vom",
        "von",
        "vor",
        "war",
        "waren",
        "warst",
        "was",
        "weil",
        "weiter",
        "welche",
        "welchem",
        "welchen",
        "welcher",
        "welches",
        "wenn",
        "werde",
        "werden",
        "wie",
        "wieder",
        "will",
        "wir",
        "wird",
        "wirst",
        "wo",
        "wollen",
        "wollte",
        "während",
        "würde",
        "würden",
        "zu",
        "zum",
        "zur",
        "zwar",
        "zwischen",
        "über",
        ]),
    "en": frozenset([
        "but",
        "all",
        "everything",
        "than",
        "so",
        "at",
        "to",
        "other",
        "different",
        "also",
        "on",
        "out",
        "from",
        "with",
        "am",
        "until",
        "are",
        "there",
        "so that",
        "then",
        "the",
        "that",
        "the same",
        "in addition",
        "your",
        "because",
        "of",
        "whose",
        "you",
        "this",
        "self",
        "here",
        "there",
        "through",
        "a",
        "some",
        "once",
        "he",
        "it",
        "something",
        "we",
        "us",
        "our",
        "can",
        "could",
        "make",
        "one",
        "my",
        "me",
        "must",
        "had to",
        "after",
        "not",
        "nothing",
        "still",
        "now",
        "only",
        "whether",
        "if",
        "or",
        "without",
        "his",
        "her",
        "its",
        "themselves",
        "she",
        "they",
        "is",
        "be",
        "such",
        "shall",
        "should",
        "rather",
        "else",
        "around",
        "for",
        "against",
        "been",
        "have",
        "has",
        "had",
        "here",
        "behind",
        "in",
        "into",
        "by",
        "each",
        "every",
        "those",
        "now",
        "want",
        "wanted",
        "while",
        "during",
        "would",
        "to",
        "too",
        "indeed",
        "between",
        "over",
        "before",
        "because",
        "further",
        "which",
        "how",
        "like",
        "again",
        "where",
        "and",
    ]),
    "fr": frozenset([
        # Conjunctions
        "et",        # and
        "ou",        # or
        "mais",      # but
        "car",       # because/for
        "donc",      # so/therefore
        "alors",     # then/so
        "puis",      # then
        "si",        # if
        "comme",     # like/as
        "parce que", # because
        "puisque",   # since/because
        "quand",     # when
        "lorsque",   # when
        "que",       # that/which
        "quoique",   # although
        "tandis que",# while/whereas

        # Articles & determiners
        "le", "la", "les",   # the
        "un", "une", "des",  # a, some
        "du", "de la",       # partitive
        "de",                # of/from
        "ce", "cet", "cette", "ces", # this, these
        "chaque",            # each
        "tout", "toute", "tous", "toutes", # all
        "autre", "autres",   # other
        "même", "mêmes",     # same
        "quel", "quelle", "quels", "quelles", # which

        # Pronouns
        "je", "j'",          # I
        "tu",                # you (singular)
        "il", "elle", "on",  # he, she, one
        "nous",              # we
        "vous",              # you (plural/polite)
        "ils", "elles",      # they
        "me", "m'", "moi",   # me
        "te", "t'", "toi",   # you
        "se", "s'", "soi",   # oneself
        "lui", "leur",       # him/her, them
        "le", "la", "les",   # him, her, them (objects)
        "y",                 # there
        "en",                # of it, some

        # Possessives
        "mon", "ma", "mes",      # my
        "ton", "ta", "tes",      # your (sing.)
        "son", "sa", "ses",      # his/her
        "notre", "nos",          # our
        "votre", "vos",          # your (pl.)
        "leur", "leurs",         # their

        # Demonstratives
        "celui", "celle", "ceux", "celles",   # the one(s)
        "ceci", "cela", "ça",                 # this/that

        # Interrogatives/relatives
        "qui",        # who
        "que",        # what/that
        "quoi",       # what
        "où",         # where
        "quand",      # when
        "comment",    # how
        "pourquoi",   # why
        "lequel", "laquelle", "lesquels", "lesquelles", # which one

        # Prepositions
        "à", "au", "aux",      # at, to
        "dans",                # in
        "par",                 # by, through
        "pour",                # for
        "sans",                # without
        "sous",                # under
        "sur",                 # on
        "avec",                # with
        "chez",                # at the place of
        "vers",                # toward
        "entre",               # between
        "devant", "derrière",  # in front of, behind
        "avant", "après",      # before, after

        # Common adverbs/particles
        "ne", "pas", "plus", "jamais", # negations
        "rien", "aucun",              # nothing, none
        "toujours",                   # always
        "encore",                      # again, still
        "déjà",                        # already
        "ici", "là",                   # here, there
        "ainsi",                       # thus
        "alors",                       # then
        "très", "trop", "bien", "mal", # very, too, well, badly
        "seulement",                   # only
        "même",                        # even

        # Auxiliary verbs (collapsed forms)
        "être",    # to be
        "avoir",   # to have
        "faire",   # to do/make
        "aller",   # to go
        "pouvoir", # can
        "vouloir", # want
        "devoir",  # must
    ])
}


CONNECTOR_WORDS = {
    "de": frozenset([
        "ohne",
        "ein",
        "eine",
        "zu",
        "bei",
        "von",
        "und",
        "an",
        "in",
        "oder",
        "für",
        "aus",
        "auf",
        "der",
        "die",
        "das",
        "des",
        "dem",
        "den",
        "dass",
        "mit",
        "über",
        "durch",
        "nach",
        "gegen",
        "entlang",
        "unter",
        "um",
        "vor",
        "hinter",
        "zwischen",
        "während",
        "außer",
        "innerhalb",
        "nahe",
        "außerhalb",
        "ausserhalbaber",
        "noch",
        "so",
        "doch",
        "als",
        "weil",
        "wenn",
        "seit",
        "da",
        "dass",
        "obwohl",
        "es sei denn",
        "bis",
        "wo",
        "solange",
        "falls",
        "obgleich",
        "unterhalb",
        "neben",
        "jenseits",
        "dieser",
        "dieses",
        "diese",
        "jene",
        "solcher",
        "solches",
        "solche",
        "welcher",
        "welches",
        "welche",
        "wessen",
        "deren",
        "dessen",
    ]),
    "en": frozenset([
        "without",
        "a",
        "a",
        "to",
        "at",
        "from",
        "and",
        "at",
        "in",
        "or",
        "for",
        "out of",
        "from",
        "on",
        "upon",
        "the",
        "of the",
        "to the",
        "that",
        "with",
        "over/about",
        "through",
        "after/to",
        "against",
        "along",
        "under",
        "around",
        "before",
        "in front of",
        "behind",
        "between",
        "during",
        "except",
        "within",
        "near",
        "outside",
        "still",
        "yet",
        "so",
        "but",
        "however",
        "than",
        "because",
        "if",
        "when",
        "since",
        "as",
        "that",
        "although",
        "unless",
        "until",
        "where",
        "as long as",
        "if/in case",
        "although",
        "below",
        "underneath",
        "beside",
        "next to",
        "beyond",
        "this",
        "that",
        "those",
        "such",
        "which",
        "whose",
    ]),
    "fr": frozenset([
        "sans",
        "un",
        "une",
        "à",
        "chez",
        "de",
        "et",
        "à",
        "dans",
        "ou",
        "pour",
        "de",
        "sur",
        "le",
        "la",
        "le",
        "du",
        "au",
        "le",
        "que",
        "avec",
        "sur",
        "par",
        "après",
        "contre",
        "le long de",
        "sous",
        "environ",
        "avant",
        "derrière",
        "entre",
        "pendant",
        "sauf",
        "à l'intérieur de",
        "près de",
        "à l'extérieur de",
    ]),
}


if __name__ == "__main__":
    pass
