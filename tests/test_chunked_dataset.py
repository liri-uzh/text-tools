import pytest
import shutil
from text_tools.data import (
    ChunkedDataset,
)  # Assuming your class is in a file named 'data.py'
...
def setup_dummy_data(temp_test_dir):
    ...
    nested_dir.mkdir()
    (nested_dir / "nested_file.md").write_text(
...
def test_save_and_load_chunked_dataset(setup_dummy_data, output_dir):
    ...
    assert hasattr(
        loaded_dataset, "tokenizer"
    )  # Ensure tokenizer is loaded and attached
...
def test_add_length_column(setup_dummy_data):
    ...
    # Add length column
    dataset_with_length = ChunkedDataset.add_length_column(
        dataset, n_processes=1
    )  # Use 1 process for consistent testing
